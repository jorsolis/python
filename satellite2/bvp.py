...
def shoot( f, a, b, z1, z2, t, tol ):
    ...
    # Because we are using the secant method to refine our estimates of z = y',
    # we don't really need all the solution of the IVP, just the last
...
if __name__ == "__main__":
    ...
    #y = shoot(f, a, b, z1, z2, t, tol)  
    xs1 = shoot( f, exp(a), 2*exp(b), 3.0, 4.0, t1, 1e-9 )
    xs2 = shoot( f, exp(a), 2*exp(b), 3.0, 4.0, t2, 1e-9 )
    ...
    cla()
    plot( t1, xs1, 'r.-', t2, xs2, 'b-', t1, x1, 'g-' )
    title( 'Shooting Method' )
    xlabel( '$t$' )
    ylabel( '$x$' )
    legend( ( '%3d points' % n1, '%3d points' % n2, 'exact' ), loc='lower right' )
#    draw()
