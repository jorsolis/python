...
# coding: utf-8
##############################################################################
############       con un termino extra -alfa/r en el potencial autogravitante
##############################################################################
...
def mainprog(ncor, Rf = 10, E= 0.034465, Vf = -0.1, fif = .001,Nf=1.,
             direct = '/home/jordi/grossN', bet = 0., R0 = 0.01, alpha = 1.5):
###########################     MESH         ##################################
...
###############          Define expressions used in variational forms       ###
    ...
    fx2 = Expression('1/sqrt(pow(x[1], 2) + pow(x[0],2))', degree=2, a0 = 1, b0= 0)#
####################         Constants           #############################
    k = Constant(E)
    g1 = Constant(0.)
...
##############          Define variational problem     #########################
    ...
    + fx*dot(grad(Phi), grad(v_2))*dx + 2.*fx*psi*Phi*v_2*dx - 2.*alpha*fx*fx2*Phi*v_2*dx + 2.*fx*k*Phi*v_2*dx \
...
mainprog(ncor, Rf= Rf, E= Ener, Vf = Vf, fif =fif, Nf = Nf, R0 = 0.1,
         bet = beta, direct = "%s/%d"%(dec,nc), alpha = 0.1) 
