...
from triaxialNFW_analisis import (mptodos_plot, mptodos,scape_vel,taus_t,
                                  mptodos_plot_l,mptodos_l, circular_vel)
import plots_jordi as pts

def triax_NFW_density(pars, x,y,z):
#  pars: (alpha = 1) pars = [G, vcrs, rs, a, b, c]
...
##################################  art chinos    
    ...
    rs = 8.1
    rhoRsun = 0.013e-1
    dens0 = Rsun*(1. + Rsun/rs)**2*rhoRsun/rs
    print(dens0)
...
################           Paramettros Potencial      ##########################
    ...
    b = 1
    c = 1
#    dens0 = 1.06e-3 # 10**10M_sun/kpc**3
#    rs = 12. #kpc   the scale radius.
    vcrs = circular_vel(rs, rs, dens0)#v_c   Circular velocity at the scale radius.
    print('vcrs = ',vcrs, 'km/s')
    r = np.linspace(0, 100, 50)
    pts.plotmultiple([r], [circular_vel(r, rs, dens0)], ['NFW'],
                     'r', 'v', 'spherical NFW', '', ylim=(0,0),
                 xlim=(0,0), logy= False, save = False, loc_leg='best',
                 ...
                 fby1=1, fby2=0, text= '', xv=[rs])
###############################################################################   
#    tiemp = 200    
    nump= 1001 
    R = 100
    vesc = scape_vel(R, dens0, rs)
    print('v_sc(r = %dkpc) = '%R, vesc)
    ri  = 0.2
    rf = 300.
    vmax = vesc/2    
#########           Escala de tiempo       ######################################
    tiemp= 10.
    vphi0 = circular_vel(R, rs, dens0)
    rho0 = R
...
#    plt.show()
    ...
    v = vphi0
    taus = 2.*np.pi*r/v 
    print('tau_s',taus/(3.14e7*1e9), 'Gyrs')
    print('tau_s',taus, 's')
###############################################################################
#    da2 = float(input('k'))
#    k = int(da2)
