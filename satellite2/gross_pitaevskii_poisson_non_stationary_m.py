...
def mainprog(rhomin, a, b, ncor, Rf = 10, Vf = -0.1, fif = .001,
             direct = '/home/jordi/gross_evolution_m', bet = 0.):
    T = 16.0# final time
...
##############          Define variational problem     #########################
    ...
    + k*fx2*dot(grad(PhiR), grad(v_2))*dx + fx*PhiR.dx(0)*v_2*dx \
    + 2.*fx2*PhiI*v_2*dx - 2.*fx2*PhiI_n*v_2*dx + 2.*k*fx2*psi*PhiR*v_2*dx \
    + 2.*k*fx2*beta*(PhiR*PhiR + PhiI*PhiI)*PhiR*v_2*dx + k*m*PhiR*v_2*dx \
    + k*fx2*dot(grad(PhiI), grad(v_3))*dx + fx*PhiI.dx(0)*v_3*dx \
    - 2.*fx2*PhiR*v_3*dx + 2.*fx2*PhiR_n*v_3*dx + 2.*k*fx2*psi*PhiI*v_3*dx \
    + 2.*k*fx2*beta*(PhiR*PhiR + PhiI*PhiI)*PhiI*v_3*dx + k*m*PhiI*v_3*dx \
...
#    input_file.close()
    ...
    u0 = Function(V)
    input_file = HDF5File(mesh2.mpi_comm(), "%s/sol0_%d/u_%d_t243.h5"%(direct,ncor,ncor), "r")
    input_file.read(u0, "solution")
    input_file.close()
...
#    solve(F2 == 0, u, [bc, bc2, bc3, bc4, bc5])
    ...
    output_file = HDF5File(mesh.mpi_comm(),
                           "%s/sol0_%d/u_%d_t%d.h5"%(direct,ncor,ncor, 244), "w")
    output_file.write(u, "solution")
    output_file.close()
###############           Solve variational problem         ###################   
#    for n in range(2,2+num_steps):
    for n in range(245,245+num_steps):
        print(t)
        t += dt
#        solve(F2 == 0, u, [bc, bc2, bc3, bc4, bc5]) 
        solve(F2 == 0, u, [bc, bc2, bc4]) 
...
def read_saved_file0(ncor, direct = '/home/jordi/gross_evolution_m'):
    ################         Read mesh         #################################
...
def read_saved_file(ncor,n, direct = '/home/jordi/gross_evolution_m'):
    ################         Read mesh         #################################
...
#################################################################################
dec = '/home/jordi/gross_evolution_m'
...
#         direct = '%s/%d/m%d'%(dec,int(beta),m), bet = beta) 

#u = read_saved_file0(ncor, direct = '%s/%d/m%d/sol0_%d'%(dec,int(beta),m, ncor))
#u.set_allow_extrapolation(True)
#psi, PhiR, PhiI = split(u)
#graf(PhiI, '', '%s/%d/m%d/sol0_%d'%(dec,int(beta),m, ncor), 
#     name='phiR', zlabel= r'Im($\Phi$)')    
#graf(PhiR, '', '%s/%d/m%d/sol0_%d'%(dec,int(beta),m, ncor),
#     name='phiR', zlabel= r'Re($\Phi$)')
for i in range(286,325,4):
    u = read_saved_file(ncor,i, direct = '%s/%d/m%d/sol0_%d'%(dec,int(beta),m, ncor))
    u.set_allow_extrapolation(True)
    plots_pyplot_2(u,Rf, ncor,i, ruta='%s/%d/m%d/sol0_%d'%(dec,int(beta),m,ncor))