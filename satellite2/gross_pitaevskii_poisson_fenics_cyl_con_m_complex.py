...
#for ncor in (7,8,3):
    ...
    mainprog(0., 1., 0., ncor, Rf= Rf, E= Ener, Vf = Vf, fif =fif, 
             direct = '%s/%d/m%d/sol0_%d/'%(dec,int(beta),int(m),ncor), bet = beta, eme = m) 
##
#    u = read_saved_file(ncor, direct = '%s/%d/m%d'%(dec,int(beta),int(m)))
#    u.set_allow_extrapolation(True)
#    plots_pyplot(u,Rf, En, expon, ncor, ruta='%s/%d/m%d'%(dec,int(beta),int(m)))
